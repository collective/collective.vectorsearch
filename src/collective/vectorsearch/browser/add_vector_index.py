    ...
    def handle_submit(self):
        ...
        if not index_id:
            IStatusMessage(self.request).addStatusMessage(
                _(u"Please provide an index ID."),
                type='error'
            )
            return self.index()
        ...
        try:
            ...
            IStatusMessage(self.request).addStatusMessage(
                _(
                    u"Vector index '${id}' added successfully.",
                    mapping={'id': index_id}
                ),
                type='info'
            )
            ...
            IStatusMessage(self.request).addStatusMessage(
                _(u"Error adding index: ${error}", mapping={'error': str(e)}),
                type='error'
            )
            return self.index()
